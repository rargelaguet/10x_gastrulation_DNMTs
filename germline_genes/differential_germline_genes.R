...
io$outdir <- paste0(io$basedir,"/results/differential/germline"); dir.create(io$outdir, showWarnings = F)
...
germline_genes[!germline_genes%in%unique(diff.dt$gene)]

# Subset germline genes
diff.dt <- diff.dt[gene%in%germline_genes]
...
p <- ggplot(to.plot, aes(x=celltype, y=N)) +
  ...
  labs(x="", y="Number of DE (germline genes)") +
...
pdf(sprintf("%s/DE_barplots_germline_genes.pdf",io$outdir), width=9, height=5)
print(p)
dev.off()
...
for (i in opts$ko.classes) {
  ...
  pdf(sprintf("%s/%s_DE_heatmap_germline_genes.pdf",io$outdir,i), width=9, height=5)
  print(p)
  dev.off()
}
...
pdf(file.path(io$outdir,"DE_heatmap_germline_genes.pdf"), width=15, height=9)
print(p)
dev.off()
