...
## START TEST ##
# args$query_samples <- c("E8_5_Dnmt1_KO_male_SIGAD8_L002")
...
# args$atlas_sce <- file.path(io$atlas.basedir,"results/trajectories/NMP/NMP_SingleCellExperiment.rds")
...
# args$outfile <- paste0(io$basedir,"/results_new/mapping/trajectories/NMP/sample_metadata_after_mapping.txt.gz")
## END TEST ##
...
mapping.dt <- mapping$mapping %>% 
  ...
  merge(meta_query[,c("cell","sample","class")],by="cell") %>%
  as.data.table

# foo <- merge(meta_query,mapping.dt[,c("cell","celltype.mapped")], by="cell", all=T)
