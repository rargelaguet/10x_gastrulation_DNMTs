...
seurat_merged@meta.data$batch[which(is.na(seurat_merged@meta.data$batch))] <- seurat_merged@meta.data$sample[which(is.na(seurat_merged@meta.data$batch))]
...
# Basing code on: http://htmlpreview.github.io/?https://github.com/satijalab/seurat-wrappers/blob/master/docs/harmony.html

seurat_merged <- NormalizeData(seurat_merged) %>% FindVariableFeatures() %>% ScaleData() %>% RunPCA(verbose = FALSE)
...
seurat_merged <- RunUMAP(seurat_merged, dims = 1:30, reduction = "harmony", umap.method = "umap-learn")
...
correct_map <- seurat_merged@reductions$harmony@cell.embeddings[rownames(md)[which(md$map=='QUERY')],]
correct_atlas <- seurat_merged@reductions$harmony@cell.embeddings[rownames(md)[which(md$map=='ATLAS')],]
