...
io$outdir <- paste0(io$basedir,"/results/differential/neural_crest")
...
diff_filt.dt <- diff.dt[gene%in%neural_crest.dt$gene]
...
for (i in unique(dt$groupA)) {
  ...
  p <- ggplot(to.plot, aes(x=logFC, y=-log10(padj_fdr+1e-100))) +
    ...
    ggrepel::geom_text_repel(data=to.plot[sig==T], aes(x=logFC, y=-log10(padj_fdr+1e-100), label=gene), size=4) +
    theme_classic() +
    theme(
